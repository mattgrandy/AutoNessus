...
url = 'https://localhost:8834' # Do not include trailing /
verify = False
token = ''
username = 'xxxxx'
password = 'xxxxx'
...
class create_menu:
	...
	def __init__(self, menu, text, other):
		...
		if self.other != 'Null':
			print('{0}'.format(menu) + (20-option_length_menu) * ' ' + '  :    {0}'.format(text) + (15-option_length_text)*' ' +  ':    {0}'.format(other))

		else:
			print('{0}' + (15-option_length_menu) * ' ' + '  :  {1}').format(menu,text)
		return
...
def login(usr, pwd):
	...
	session = url + '/session'
	request_post = requests.post(session, data = login, verify=False)
